...
Player::Player(WorldSession* session) : Unit(),
    ...
    m_honorMgr(this), m_personalXpRate(-1.0f), m_isStandUpScheduled(false), m_foodEmoteTimer(0), m_showLowLevelQuests(false)
{
...
int32 Player::CalculateReputationGain(ReputationSource source, int32 rep, int32 faction, uint32 creatureOrQuestLevel)
{
    ...
    return rand_dither(configRate * rep * percent / 100.0f);
}
...
void Player::SendPetSkillWipeConfirm() const
{
    ...
    packet->cost = static_cast<uint32>(pet->GetResetTalentsCost() * sWorld.getConfig(CONFIG_FLOAT_PET_UNTRAIN_COST_FACTOR));
    GetSession()->SendPacket(std::move(packet));
#endif
}
